...
def extract_column_texts():
    ...
    column_texts = [
        f"Colonna '{row['field_name']}' ({row['field_description'] or 'nessuna descrizione'}) "
        f"della tabella '{row['table_id']}' ({row['table_description'] or 'nessuna descrizione'})"
...
def extract_table_texts():
    ...
    grouped_fields = df_fields.groupby("table_id").apply(
        lambda g: ", ".join(f"{row['field_name']} ({row['field_description'] or 'nessuna descrizione'})"
    ...
    table_texts = [
        f"Tabella '{row['table_id']}': {row['table_description'] or 'nessuna descrizione'} - "
        f"contiene colonne: {row['column_descriptions'] or 'nessuna colonna'}"
