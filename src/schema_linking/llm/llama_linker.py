...
from schema_linking.prompt.prompt_builder import (
    build_table_prompt, build_column_prompt, build_column_filtering_prompt
)
...
def llama_filter_columns_by_keywords(question: str, column_texts: list[str], keywords: list[str]) -> list[str]:
    prompt = build_column_filtering_prompt(question, column_texts, keywords)
    output = send_prompt(prompt)
    return output